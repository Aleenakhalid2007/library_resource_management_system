...
struct SearchFilter {
    ...
    double maxRating;
    bool availableOnly;
    int resourceType;  // 0 = Any, 1 = Physical Books, 2 = Digital/EBooks

    SearchFilter() : minYear(0), maxYear(3000), minRating(0.0), 
                    maxRating(5.0), availableOnly(false), resourceType(0) {}
};
...
public:
    ...
    static std::vector<Resource*> searchPhysicalBooks(const std::vector<Resource*>& resources);
    static std::vector<Resource*> searchDigitalBooks(const std::vector<Resource*>& resources);
