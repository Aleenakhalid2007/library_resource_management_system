...
LibraryManager::LibraryManager(UserManager& um, ReportGenerator& rg)
    : resourceFilename_("resources.txt"), recordFilename_("borrow_records.txt"),
      ...
      reservationFilename_("reservations.txt"), borrowRequestFilename_("borrow_requests.txt"),
      nextNotificationID_(1), userManager_(um), reportGen_(rg) {
    loadResources();
    if (resources_.empty())
        seedSampleResources();
...
void LibraryManager::displayIssuedOrOverdueResourceReport() const {
    ...
    for (auto* rec : records_) {
        ...
        if (isOverdue) {
            overdueDays = difftime(now, dueTime) / (24 * 60 * 60);
        }
        ...
        cout << "Status      : " << (isOverdue ? "OVERDUE" : "Issued") << "\n";
        if (isOverdue)
            cout << "Overdue Days: " << static_cast<int>(overdueDays) << " | Fine Estimate: Rs. " << (overdueDays * 10.0) << "\n";
        cout << "---------------------------\n";
    }
...
// ============= BORROW REQUEST SYSTEM (24-hour physical book verification) =============

int LibraryManager::generateBorrowRequestID() const {
    int maxID = 0;
    for (const auto* req : borrowRequests_) {
        if (req && req->getRequestID() > maxID)
            maxID = req->getRequestID();
    }
    return maxID + 1;
}

BorrowRequest* LibraryManager::requestBorrowPhysicalBook(int userID, int resourceID,
                                                         const string& requestDate) {
    Validator::validateID(userID);
    Validator::validateID(resourceID);
    Validator::validateDate(requestDate);
    ...
    Member* member = dynamic_cast<Member*>(user);
    if (!member)
        throw AuthorizationException("Only members can request to borrow books.");
    ...
    if (!resource->isPhysical())
        throw ResourceStateException("This resource is not a physical book. Use direct access for digital resources.");

    if (!member->canBorrow())
        throw ResourceStateException("Borrow limit reached!");

    // Create and save borrow request
    BorrowRequest* request = new BorrowRequest();
    request->createRequest(generateBorrowRequestID(), userID, resourceID, requestDate);
    borrowRequests_.push_back(request);
    
    saveBorrowRequests();
    userManager_.saveToFile();

    // Send notification
    string message = "Your borrow request for '" + resource->getTitle() +
                     "' has been submitted. Admin will review and approve within 24 hours.";
    sendNotification(userID, resourceID, message, NotificationType::BOOK_AVAILABLE);

    cout << "Borrow request submitted successfully!\n";
    cout << "Request ID: " << request->getRequestID() << "\n";
    cout << "Please wait for admin approval. You will have 24 hours to visit the library.\n";

    return request;
}

void LibraryManager::approveBorrowRequest(int requestID, const string& approvalDate,
                                         int daysValid) {
    BorrowRequest* request = findBorrowRequest(requestID);
    if (!request)
        throw NotFoundException("Borrow request not found.");

    if (request->getStatus() != BorrowRequestStatus::PENDING)
        throw ResourceStateException("Only pending requests can be approved.");

    // Calculate expiry date (24 hours from now, simplified to next day)
    // Format: YYYY-MM-DD, so we just add 1 to the day
    string expiryDate = approvalDate;
    int day = stoi(expiryDate.substr(8, 2));
    expiryDate[8] = '0' + (day / 10);
    expiryDate[9] = '0' + (day % 10 + 1);
    if (day + 1 > 31) {
        expiryDate[8] = '0' + 0;
        expiryDate[9] = '1';
    }

    request->approveRequest(approvalDate, expiryDate);
    saveBorrowRequests();

    // Send notification to member
    int userID = request->getUserID();
    Resource* resource = searchByID(request->getResourceID());
    if (resource) {
        string message = "Your borrow request for '" + resource->getTitle() +
                         "' has been approved! Please visit the library within 24 hours (before " +
                         expiryDate + ") with your ID to complete the borrow.";
        sendNotification(userID, request->getResourceID(), message, NotificationType::BORROW_APPROVED);
    }

    cout << "Borrow request approved!\n";
    cout << "Member has until " << expiryDate << " to visit library for verification.\n";
}

void LibraryManager::verifyAndCompleteBorrow(int requestID, int userID, int resourceID,
    ...
    if (request->getStatus() != BorrowRequestStatus::APPROVED)
        throw ResourceStateException("Only approved requests can be verified.");

    if (!request->isValid(verificationDate))
        throw ResourceStateException("Borrow request has expired! Please request again.");

    // Verify user and resource IDs match
    if (request->getUserID() != userID)
        throw AuthorizationException("User ID does not match the request.");

    if (request->getResourceID() != resourceID)
        throw ValidationException("Resource ID does not match the request.");

    // Mark as verified
    request->verifyAndComplete(verificationDate, verificationMethod);

    // Now create the actual borrow record
    try {
        // Get due date (14 days from verification)
        string dueDate = verificationDate;
        int day = stoi(dueDate.substr(8, 2)) + 14;
        dueDate[8] = '0' + (day / 10);
        dueDate[9] = '0' + (day % 10);

        borrowResource(userID, resourceID, verificationDate, dueDate);
        ...
        string message = "Your identity has been verified! Book '" +
                        searchByID(resourceID)->getTitle() + "' is now borrowed.";
        sendNotification(userID, resourceID, message, NotificationType::BORROW_APPROVED);

        cout << "Member identity verified and book issued successfully!\n";
    }
    catch (const exception& e) {
        throw runtime_error(string("Failed to complete borrow: ") + e.what());
    }
}

BorrowRequest* LibraryManager::findBorrowRequest(int requestID) const {
    for (auto* request : borrowRequests_) {
...
vector<BorrowRequest*> LibraryManager::getPendingBorrowRequests() const {
    vector<BorrowRequest*> pending;
    for (auto* request : borrowRequests_) {
        if (request && request->getStatus() == BorrowRequestStatus::PENDING)
            pending.push_back(request);
    }
    return pending;
}

vector<BorrowRequest*> LibraryManager::getApprovedBorrowRequests() const {
    vector<BorrowRequest*> approved;
    for (auto* request : borrowRequests_) {
        if (request && request->getStatus() == BorrowRequestStatus::APPROVED)
            approved.push_back(request);
    }
    return approved;
}

void LibraryManager::loadBorrowRequests() {
    ifstream file(borrowRequestFilename_);
    if (!file.is_open()) {
    ...
    while (getline(file, line)) {
        if (!line.empty()) {
            BorrowRequest* request = BorrowRequest::deserialize(line);
            if (request)
                borrowRequests_.push_back(request);
        }
    }
...
void LibraryManager::saveBorrowRequests() const {
    ofstream file(borrowRequestFilename_);
    if (!file.is_open())
        return;

    for (const auto* request : borrowRequests_) {
        if (request)
...
bool LibraryManager::accessEBook(int userID, int resourceID, string& eBookURL) {
    Validator::validateID(userID);
    Validator::validateID(resourceID);
    ...
    if (!resource->isDigital())
        throw ResourceStateException("This resource is not a digital eBook.");

    // For eBooks, we directly provide access without creating a borrow record
    EBook* ebook = dynamic_cast<EBook*>(resource);
    if (!ebook)
        throw runtime_error("Failed to cast to EBook.");

    eBookURL = ebook->getDownloadLink();

    // Send notification about eBook access
    string message = "You now have access to '" + resource->getTitle() +
                     "'. Enjoy reading! No return date required for eBooks.";
    sendNotification(userID, resourceID, message, NotificationType::BORROW_APPROVED);

    cout << "eBook access granted!\n";
    cout << "Title: " << resource->getTitle() << "\n";
    cout << "Format: " << ebook->getFormat() << "\n";
    cout << "Access URL: " << eBookURL << "\n";
    cout << "Note: No return date. You can access anytime!\n";
...
LibraryManager::~LibraryManager() {
    ...
    for (auto r : borrowRequests_) delete r;
