...
vector<Resource*> AdvancedSearch::searchPhysicalBooks(const vector<Resource*>& resources) {
    vector<Resource*> results;
    for (auto* res : resources) {
        if (res && res->isPhysical()) {
            results.push_back(res);
        }
    }
    return results;
...
vector<Resource*> AdvancedSearch::searchDigitalBooks(const vector<Resource*>& resources) {
    vector<Resource*> results;
    for (auto* res : resources) {
        if (res && res->isDigital()) {
            results.push_back(res);
        }
    }
    return results;
...
bool AdvancedSearch::matchesFilter(Resource* resource, const SearchFilter& filter) {
    ...
    // Resource type filter (1 = Physical, 2 = Digital)
    if (filter.resourceType == 1 && !resource->isPhysical()) {
        return false;
    }
    if (filter.resourceType == 2 && !resource->isDigital()) {
        return false;
    }
