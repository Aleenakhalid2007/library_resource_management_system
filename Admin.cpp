...
string Admin::getCreatedAt() const {
    return createdAt_;
}
...
void Admin::changePassword(const string& oldPassword, const string& newPassword) {
    ...
    if (!salt.empty()) {
        // Hashed password - verify old password
        if (!SecurityUtils::verifyPassword(oldPassword, salt, storedHash)) {
            throw runtime_error("Current password is incorrect.");
        }
        password_ = SecurityUtils::createSecurePassword(newPassword);
    } else {
        // Plaintext password (legacy) - just replace
